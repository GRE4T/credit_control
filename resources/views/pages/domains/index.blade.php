...
<div class="breadcrumb">
  <ul class="d-flex align-items-center">
    ...
    <li class="h5 bold"> Dominios </li>
  </ul>
</div>
<div class="row mb-2">
  <div class="col">
    <a class="btn btn-success" href="{{ route('domains.create')}}">Añadir nuevo</a>
  </div>
</div>
<div class="card mb-4">
  ...
  <div class="card-body">
    <div class="table-responsive">
      <table id="table_domains" class="table table-borderless table-hover">
        <thead>
          <tr>
            <th scope="col">#</th>
...
<script type="text/javascript">
    ...
    $('#table_domains').DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render(data) {
            return `
              <a href="{{ url('domains') }}/${data}/edit" class="text-success mr-2">
                  <i class="nav-icon i-Pen-2 font-weight-bold"></i>
              </a>
              <a href="javascript:void(0)" class="text-danger mr-2" onclick="deleteClient(${data})">
  ...
  function deleteClient(id) {
    swal({
      ...
      axios.delete("{{ url('domains') }}/" + id, null, {
          header: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          }
        })
        .then((res) => {
          //$('#table_domains').ajax.reload();
          swal(
            '¡Eliminado!',
