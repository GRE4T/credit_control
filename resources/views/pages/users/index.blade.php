...
<script type="text/javascript">
        ...
        },
        {
          ...
          render(data,type,row) {
            return `
                    ...
                    <a href="javascript:void(0)" class="text-danger mr-2" onclick="activeUser(${data}, ${row.active})">
  ...
  function activeUser(id, active) 
  {
    Swal.fire({
      ...
      confirmButtonText: active ? 'Si, desactivar!': 'Si, activar!',
      cancelButtonText: 'Cancelar' 
    }).then((result) => {
      if (result.isConfirmed) {
        axios.delete("{{ url('users') }}/" + id, null, {
          header: {
        ...
        .then((res) => {
          ...
          Swal.fire(
            '¡Estado actualizado!',
            active ? 'Usuario desactivado': 'Usuario activado',
            'success'
          );
        })
        .catch((error) => {
          if(error) {
            Swal.fire(
              'Cancelado',
              'Este registro no puede ser actualizado :(',
              'error'
            )
          }
        })
